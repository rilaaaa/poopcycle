...
export default function Auth({ onAuthSuccess, lang, setLang, theme, setTheme }: AuthProps) {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.code === 'auth/user-not-found' || err.message?.includes('user-not-found')) localErr = lang === 'en' ? 'Email not registered.' : 'Email tidak terdaftar.';
      if (err.code === 'auth/wrong-password' || err.message?.includes('wrong-password')) localErr = lang === 'en' ? 'Incorrect password.' : 'Password salah.';
      if (err.code === 'auth/invalid-credential' || err.message?.includes('invalid-credential') || err.message?.includes('auth/invalid-credential')) {
        localErr = lang === 'en' ? 'Incorrect email or password.' : 'Email atau password salah.';
      }
      if (err.code === 'auth/email-already-in-use' || err.message?.includes('email-already-in-use')) localErr = lang === 'en' ? 'Email is already in use.' : 'Email sudah digunakan.';
      if (err.code === 'auth/invalid-email' || err.message?.includes('invalid-email')) localErr = lang === 'en' ? 'Invalid email format.' : 'Format email tidak valid.';
      if (err.code === 'auth/weak-password' || err.message?.includes('weak-password')) localErr = lang === 'en' ? 'Password is too weak.' : 'Password terlalu lemah.';
  ...
  const handleGuestLogin = async () => {
    ...
    try {
      ...
      // Initialize guest user profile in Firestore
      const userDocRef = doc(db, 'users', user.uid);
      const userDoc = await getDoc(userDocRef);
      ...
      if (!userDoc.exists()) {
        await setDoc(userDocRef, {
          uid: user.uid,
          email: '',
          displayName: lang === 'en' ? 'Guest User' : 'Tamu',
          dailyWaterGoal: 8,
          notificationsEnabled: true,
          registeredAt: new Date().toISOString()
        });
      ...
      console.error(err);
      setError((lang === 'en' ? 'Failed to sign in as guest: ' : 'Gagal masuk sebagai tamu: ') + err.message);
    } finally {
      setLoading(false);
    }
  };
          ...
          {!isReset && (
            <div className="space-y-3">
              ...
              <button
                ...
                disabled={loading}
                className="w-full py-2.5 px-4 border border-teal-200 dark:border-teal-900 rounded-xl text-sm font-bold text-teal-700 dark:text-teal-400 bg-teal-50/50 dark:bg-teal-950/20 hover:bg-teal-100/50 dark:hover:bg-teal-950/40 focus:outline-none focus:ring-2 focus:ring-teal-500 transition duration-200 flex items-center justify-center gap-2 shadow-sm disabled:opacity-50 cursor-pointer"
              >
                <Sparkles className="h-4.5 w-4.5 text-teal-600 dark:text-teal-400" />
                {lang === 'id' ? 'Masuk sebagai Tamu' : 'Sign in as Guest'}
              </button>
            </div>
          )}
